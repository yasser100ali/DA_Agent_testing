...
class DeepInsightsAgent:
    ...
    def deepinsights(self):
        ...
        with st.expander('Thinking of plan...'):
            json_plan = agent.json_agent(orchestrator_job)
        ...
        with st.expander('Task Execution and Evidence Gathering', expanded=False):    
            for step_number, step in enumerate(json_plan.values(), start=1):
                ...
                result, code = agent.coder(system_prompt, real_input=real_input)
        ...
        report = agent.reporter(reporter_role, reporter_input)
        
        def report_coder(user_input, report, features_list):
            system_prompt = """
            ...
            Here is the data used to get the report: {str(results)}
